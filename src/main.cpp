...
#if defined(NDEBUG)
# error "Recycle_Coin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x7f740f5d4de8c5af0567eb1986f02bfa87cfdf8e2e18a49a7d6ebbd7d600e401");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Recycle_Coin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Recycle_Coin Signed Message:\n";
...
{
    // Recycle_Coin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1680000); // Recycle_Coin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Recycle_Coin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Recycle_Coin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Recycle_Coin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Recycle_Coin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3d8f82193e585c62d6f20962676e2eeb27588448fed5c78f283726e605800669");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "28/Oct/1923 Efendiler yarın Cumhuriyeti ilan edeceğiz.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041f4d064b79cec1d3f6c417b6480808351488dce22b1ac33d841e9c4382e47230f9a8960be6c3829ee81cc8a21639bf0ca9e36b2970b5a52d866c20700d12d2aa") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1665753249;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084642393;

        if (fTestNet)
        {
            block.nTime    = 1665753207;
            block.nNonce   = 387037203;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xaac0670d0affe6b249ed16a7a8a99cd8b1c6f11a78beeac7615d049a8896839e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc4, 0xb2, 0xdf }; // Recycle_Coin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Recycle_CoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Recycle_CoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Recycle_CoinMiner(CWallet *pwallet)
{
    printf("Recycle_CoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Recycle_Coin-miner");
    ...
    try { loop {
        ...
        printf("Running Recycle_CoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Recycle_CoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Recycle_CoinMiner, pwallet));
}
